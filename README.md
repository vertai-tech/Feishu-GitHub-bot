...
通知按角色路由到**相关的人本人**（受理人 / PR 作者 / reviewer），并给对应飞书人**建任务**。找不到人可投递时（无受理人、未绑定、发送失败）才**回退给管理员** `notify_id`（默认按 `union_id` 私聊；也可配成群 `chat_id`）。不做固定抄送。

- **PR / Issue 指派受理人（assigned / opened 带受理人）** → 通知受理人本人，建任务；未绑定回退管理员。
- **请求 review（review_requested）** → 通知被请求的 reviewer，建任务。
- **PR 有新评论（issue_comment on PR）** → 通知 PR **作者**「您的 PR 有新评论」。
- **Issue 新评论（issue_comment on issue）** → 通知该 Issue 的受理人。
- **审查者提交 review（pull_request_review）** → 通知 PR 作者「审查已完成」。
- **PR 合并/关闭（closed）** → 通知 PR 作者与受理人，完成相关任务。
- **Issue 新建/关闭/重开（issues）** → 通知受理人（无则回退管理员）。

每张通知卡片都带「添加到任务」按钮，任何收到的人可一键加到自己的飞书任务。

reviewer / 受理人映射靠**用户自助绑定**：私聊机器人 → 收到一张卡片 → 填 GitHub 用户名提交 → 存入飞书多维表格。
...
- 事件：勾选 **Pull requests**、**Issues**、**Issue comments**、**Pull request reviews**（审查完成通知作者用）
